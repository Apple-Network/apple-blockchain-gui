...
export function getUserDataDir(): string {
  const appleRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(appleRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const appleRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${appleRootUserDataPath}`);
  app.setPath('userData', appleRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(appleRootUserDataPath, 'Local Storage', 'leveldb');
