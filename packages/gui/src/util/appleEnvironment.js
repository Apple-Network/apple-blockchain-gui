...
const PY_DIST_FILE = 'apple_daemon';
const PY_FOLDER = '../../../apple/daemon';
...
const getAppleVersion = () => {
  let version = null;
  const exePath = getExecutablePath('apple');
  // first see if we can get a apple exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or apple is on the path
    try {
      version = childProcess
