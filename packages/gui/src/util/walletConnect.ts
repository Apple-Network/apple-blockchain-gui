...
const log = initDebug('apple-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `apple_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.apple;
    if (!requiredNamespace) {
      throw new Error('Missing required apple namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['apple:testnet', 'apple:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `apple:${instance}:${fingerprint}`);
