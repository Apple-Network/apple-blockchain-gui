import { useGetFarmedAmountQuery } from '@apple-network/api-react';
import {
  useCurrencyCode,
  mojoToAppleLocaleString,
  useLocale,
  CardSimple,
  FormatLargeNumber,
  Tooltip,
...
function FarmingRewardsHistoryCards() {
  ...
  const totalAppleFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total APPLE Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    if (data && data.farmedAmount) {
        ...
        <StyledTable>
          <tbody>
            <tr>
              ...
              <td>
                {mojoToAppleLocaleString(data.poolRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToAppleLocaleString(data.farmerRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToAppleLocaleString(data.feeAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
      ...
      cardValue = (
        <Tooltip title={title}>
          {mojoToAppleLocaleString(data.farmedAmount, locale)}
          &nbsp;
          {currencyCode}
        </Tooltip>
      );
    ...
    return <CardSimple title={<Trans>Total APPLE Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
