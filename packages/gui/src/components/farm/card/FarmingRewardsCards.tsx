import { useGetBlockchainStateQuery, useGetTotalHarvestersSummaryQuery } from '@apple-network/api-react';
import { State, CardSimple, useCurrencyCode, mojoToAppleLocaleString, useLocale } from '@apple-network/core';
...
const MOJO_PER_APPLE = 1_000_000_000_000;
const BLOCKS_PER_YEAR = 1_681_920; // 32 * 6 * 24 * 365
function getBlockRewardByHeight(height: number) {
  if (height === 0) {
    return 21_000_000 * MOJO_PER_APPLE;
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
    return 2 * MOJO_PER_APPLE;
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return 1 * MOJO_PER_APPLE;
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return 0.5 * MOJO_PER_APPLE;
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return 0.25 * MOJO_PER_APPLE;
  }

  return 0.125 * MOJO_PER_APPLE;
}
...
function FarmingRewardsCards() {
  ...
  const estimatedDailyAPPLECard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated daily APPLE</Trans>} state={state} />;
    }

    const estimatedDailyAPPLE = new BigNumber(86_400)
    ...
    return (
      <CardSimple
        title={<Trans>Estimated daily APPLE</Trans>}
        value={
          <>
            {mojoToAppleLocaleString(estimatedDailyAPPLE, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  const estimatedMonthlyAPPLECard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated monthly APPLE</Trans>} state={state} />;
    }

    const estimatedMonthlyAPPLE = new BigNumber(86_400 * 31)
      .div(expectedTimeToWinSeconds)
      .multipliedBy(getBlockRewardByHeight(data.peak.height))
      .dp(0);
    ...
    return (
      <CardSimple
        title={<Trans>Estimated monthly APPLE</Trans>}
        value={
          <>
            {mojoToAppleLocaleString(estimatedMonthlyAPPLE, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  return (
    <Box>
      ...
      <Grid spacing={2} alignItems="stretch" container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          {estimatedDailyAPPLECard}
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
          {estimatedMonthlyAPPLECard}
        </Grid>
      </Grid>
    </Box>
  );
