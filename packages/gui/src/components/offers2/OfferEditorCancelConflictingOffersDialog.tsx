import { ConfirmDialog, Flex } from '@apple-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'APPLE' && assetStatus.type === 'APPLE') {
      // assetId might be either 'APPLE' or 'UNKNOWN'(maybe fee) but here both are treated as just a APPLE spending
