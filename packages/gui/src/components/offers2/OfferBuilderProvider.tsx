import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@apple-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@apple-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@apple-network/api-react';
import { catToMojo, appleToMojo } from '@apple-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedAPPLE = useWatch({
    name: 'requested.apple',
  });

  const offeredAPPLE = useWatch({
    name: 'offered.apple',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedAPPLE ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('apple', appleToMojo(amount))),
    ...(requestedTokens ?? [])
