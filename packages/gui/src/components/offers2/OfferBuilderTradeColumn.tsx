import { Color, Flex } from '@apple-network/core';
import { Offering, Requesting } from '@apple-network/icons';
...
import OfferBuilderAPPLESection from './OfferBuilderAPPLESection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showAPPLE = !readOnly || !!apple.length;
  ...
  const mutedAPPLE = nfts.length || tokens.length;
  const mutedTokens = apple.length || nfts.length;
  const mutedNFTs = apple.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showAPPLE && <OfferBuilderAPPLESection name={`${name}.apple`} offering={offering} muted={mutedAPPLE} />}
