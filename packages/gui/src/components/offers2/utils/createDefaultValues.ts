...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (APPLE), indicates whether a token or CAT has a default entry
 ...
 * for creating an APPLE offer, a CAT offer, or an NFT offer. APPLE and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      apple: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
