import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@apple-network/api-react';
import {
  UnitFormat,
  CardStep,
  ButtonLoading,
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit({ fee }: SubmitData) {
    try {
      ...
      const feeMojos = appleToMojo(fee);
        ...
        <Flex flexDirection="column" gap={3}>
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat value={mojoToAppleLocaleString(balance)} display="inline" state={State.SUCCESS} /> to{' '}
                {address}
              </Trans>
            </Typography>
