import { toBech32m, toSnakeCase } from '@apple-network/api';
import { useSignMessageByAddressMutation, useSignMessageByIdMutation } from '@apple-network/api-react';
import { Button, Card, Flex, Form, TextField, useOpenDialog, useShowError } from '@apple-network/core';
  ...
  async function handleSignById(messageToSign: string, id: string, address: string) {
    ...
    if (error) {
      ...
      if (missingNFTMatch || missingDIDMatch) {
        ...
        const entityId = toBech32m(entityPuzzleHash, missingNFTMatch ? 'nft' : 'did:apple:');
