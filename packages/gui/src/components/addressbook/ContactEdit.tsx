// import { useGetNFTInfoQuery } from '@apple-network/api-react';
import { fromBech32m } from '@apple-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@apple-network/core';
...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[3] === '1') {
          if (entry.address.slice(0, 5).toLowerCase() !== 'apple') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[6] === '1') {
          if (entry.address.slice(0, 6).toLowerCase() !== 'tapple') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 11).toLowerCase() !== 'did:apple:') {
          throw new Error();
