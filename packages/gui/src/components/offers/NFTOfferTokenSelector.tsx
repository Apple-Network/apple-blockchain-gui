...
import type { CATToken, Wallet } from '@apple-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@apple-network/api-react';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const appleWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Apple',
        symbol: currencyCode,
        displayName: `Apple (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...appleWalletSelection, ...catOptions];
