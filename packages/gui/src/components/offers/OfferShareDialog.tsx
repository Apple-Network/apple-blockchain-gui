...
const log = debug('apple-gui:offers');
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50_029, // Pairing doesn't exist e.g. APPLE/RandoCoin
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tapple' : 'apple'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
