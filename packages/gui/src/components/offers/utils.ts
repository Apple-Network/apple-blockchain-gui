...
export function determineNFTOfferExchangeType(
  ...
  return nftOffered
    ? NFTOfferExchangeType.NFTForAPPLE
    : NFTOfferExchangeType.APPLEForNFT;
}
...
export function getNFTPriceWithoutRoyalties(
  ...
  // NFTs can only be exchanged for APPLE currently
  const amountInMojos = offerAssetAmountForAssetId('apple', summary);
  if (amountInMojos === undefined) {
    return undefined;
  }
  return mojoToApple(amountInMojos).toNumber();
}
