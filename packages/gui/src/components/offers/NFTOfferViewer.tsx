import { OfferSummaryRecord, OfferTradeRecord } from '@apple-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@apple-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['apple', 'tapple'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.APPLE: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const appleMakerFee = mojoToApple(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(appleMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.APPLE
        ? appleToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
