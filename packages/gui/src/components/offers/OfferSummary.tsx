...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToApple, mojoToCAT } from '@apple-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['apple', 'tapple'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToApple(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['apple', 'tapple'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToApple(takerEntries[0][1])
