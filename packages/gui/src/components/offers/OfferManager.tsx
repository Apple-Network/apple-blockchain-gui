...
import { Offers } from '@apple/icons';
import {
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: appleFee } = methods.getValues();

    const fee = cancelWithTransaction ? appleToMojo(appleFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TAPPLE';
        ...
        <Grid container>
          <Grid xs={12} md={6} lg={5} item>
            <CardHero>
              ...
              <Typography variant="body1">
                <Trans>
                  Create an offer to exchange assets including APPLE, tokens, and
