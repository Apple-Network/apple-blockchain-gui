...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@apple/api-react'
...
export default function Block() {
  ...
  const baseFarmerReward = mojoToApple(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const appleFees = blockRecord.fees !== undefined
    ? mojoToApple(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://explorer.applecoin.in/blocks/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://explorer.applecoin.in/blocks/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
