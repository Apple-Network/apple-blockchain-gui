import { toBech32m } from '@apple-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@apple-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToApple(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToApple(calculateBaseFarmerReward(blockRecord.height));

  const appleFees = blockRecord.fees !== undefined ? mojoToApple(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: appleFees ? `${appleFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the Apple blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
