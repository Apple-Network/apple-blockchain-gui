...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'APPLE').toUpperCase();
  const isTestnet = currencyCode === 'TAPPLE';
  ...
  const spendableBalance = mojoToAppleLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet0-faucet.applecoin.in' : 'faucet.applecoin.in'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Apple Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
