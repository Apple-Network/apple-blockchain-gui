import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@apple-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, appleToMojo, mojoToApple } from '@apple-network/core';
import { ConnectCheckmark } from '@apple-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToApple(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = appleToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
