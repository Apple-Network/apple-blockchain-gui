import { Button, Flex, useOpenDialog, appleToMojo } from '@apple-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeApple = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeApple) {
        const feeMojos = feeApple ? appleToMojo(feeApple).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
