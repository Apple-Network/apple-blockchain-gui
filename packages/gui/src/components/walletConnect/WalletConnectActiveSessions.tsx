import { Flex, Loading, TooltipIcon } from '@apple-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `apple_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.apple?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the Apple application may not support some commands requested by the
