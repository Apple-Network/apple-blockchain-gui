...
import { TransactionType } from '@apple/api';
import type { Transaction } from '@apple/api';
import { mojoToApple, blockHeightToTimestamp } from '@apple/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToApple(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToApple(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToApple(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToApple(start).toString, // bignumber is not supported by react
    });
  });
