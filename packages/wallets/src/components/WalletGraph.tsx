import { TransactionType, WalletType } from '@apple-network/api';
import type { Transaction } from '@apple-network/api';
import { useGetWalletBalanceQuery } from '@apple-network/api-react';
import { Color, mojoToApple, mojoToCAT, blockHeightToTimestamp } from '@apple-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToApple(start)).toNumber()
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToApple(start).toString(), // bignumber is not supported by react
    });
  });
