...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToAppleLocaleString, mojoToCATLocaleString } from '@apple/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@apple/api-react';
import styled from 'styled-components';
import type { Row } from '@apple/core';
import { WalletType, TransactionType } from '@apple/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToAppleLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToAppleLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
