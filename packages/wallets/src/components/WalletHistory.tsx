import { WalletType, TransactionType, TransactionTypeFilterMode, toBech32m } from '@apple-network/api';
...
import type { Row } from '@apple-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            <FormatLargeNumber
              value={[WalletType.CAT, WalletType.CRCAT].includes(type) ? mojoToCAT(row.amount) : mojoToApple(row.amount)}
            />
          </strong>
          &nbsp;
          {metadata.unit}
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToApple(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
