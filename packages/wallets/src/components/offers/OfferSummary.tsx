...
import { type OfferSummaryRecord  } from '@apple/api';
import {
  Flex,
  StateColor,
  mojoToApple,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'apple' ? Number(mojoToApple(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'apple' ? Number(mojoToApple(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
