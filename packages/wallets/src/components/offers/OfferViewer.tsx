...
import { OfferSummaryRecord, OfferTradeRecord, OfferCoinOfInterest, WalletType } from '@apple/api';
import { useCheckOfferValidityMutation, useTakeOfferMutation } from '@apple/api-react';
import {
  colorForOfferState,
...
function OfferDetails(props: OfferDetailsProps) {
      ...
      field: (coin: OfferCoinOfInterest) => {
        return (
          <Typography variant="body2">
            <Flex flexDirection="row" flexGrow={1} gap={1}>
              {mojoToAppleLocaleString(coin.amount)}
            </Flex>
          </Typography>
        )
      },
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://explorer.applecoin.in/coin/${coin.parentCoinInfo}`)}
            >
              {coin.parentCoinInfo}
            </Link>
          </Tooltip>
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://explorer.applecoin.in/blocks/${coin.puzzleHash}`)}
            >
              {coin.puzzleHash}
            </Link>
          </Tooltip>
  ...
  async function handleAcceptOffer(formData: any) {
    const { fee } = formData;
    const feeInMojos = fee ? Number.parseFloat(appleToMojo(fee)) : 0;
