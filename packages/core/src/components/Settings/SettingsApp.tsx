...
import { Farming } from '@apple-network/icons';
...
export default function SettingsApp(props: SettingsAppProps) {
  ...
  async function handleOpenFAQURL(): Promise<void> {
    try {
      ...
      await shell.openExternal('https://github.com/Apple-Network/apple-blockchain/wiki/FAQ');
    } catch (error: any) {
      showError(error);
    }
  }
  ...
  async function handleOpenSendFeedbackURL(): Promise<void> {
    try {
      const { shell } = window as unknown as { shell: Shell };
      await shell.openExternal('https://feedback.applecoin.in/lightwallet');
    } catch (error: any) {
      showError(error);
    }
  }
    ...
    <Flex flexDirection="column" gap={3}>
      ...
      <Flex flexDirection="column" gap={1}>
        <Flex flexDirection="row" gap={1}>
          <SettingsLabel>
            <Trans>Apple Application Version:</Trans>
          </SettingsLabel>
          {version && (
