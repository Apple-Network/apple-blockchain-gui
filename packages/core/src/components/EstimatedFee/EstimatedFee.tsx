import { useGetFeeEstimateQuery } from '@apple-network/api-react';
...
import mojoToAppleLocaleString from '../../utils/mojoToAppleLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendAPPLE = 'send_apple_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToAppleLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const appleFee = mojoToAppleLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, appleFee);
        }
      }
    }
