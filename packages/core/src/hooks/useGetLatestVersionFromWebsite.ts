import { useLocalStorage } from '@apple-network/api-react';
...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.applecoin.in/latest/latest.json'
  );
    ...
    ipcRenderer.invoke('fetchHtmlContent', latestVersionURL).then((obj: any) => {
      try {
        ...
        /* we don't need to handle error here, if we are unable to fetch version number
           from applecoin.in, we just ignore showing reminder dialog */
      }
    });
  }, [latestVersionURL]);

  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.applecoin.in/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.applecoin.in/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.applecoin.in/').toString() : undefined;
