...
import { AlertDialog, Card, Flex } from '@apple/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const appleper_input = ip_parsed.limit;
    ...
    const appleper_value = Number.parseInt(Number(appleper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (apple per interval):{' '}
                  {mojo_to_apple_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = apple_to_mojo(fee_input.value);
