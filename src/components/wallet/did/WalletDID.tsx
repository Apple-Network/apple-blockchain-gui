...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@apple/core';
import {
...
import { mojo_to_apple_string } from '../../../util/apple';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_apple_string(props.balance)} TAPPLE
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'apple_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'apple_addr://') {
