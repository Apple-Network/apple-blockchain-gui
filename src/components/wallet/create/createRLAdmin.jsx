...
import { AlertDialog } from '@apple/core';
import {
  Typography,
  Button,
  Box,
...
import { apple_to_mojo } from '../../../util/apple';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      appleper_input.value === '' ||
      Number(appleper_input.value) === 0 ||
      !Number(appleper_input.value) ||
      isNaN(Number(appleper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const appleper = apple_to_mojo(appleper_input.value);
    const appleper_value = Number.parseInt(Number(appleper));
    const userpubkey = userpubkey_input.value;
    const amount = apple_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = apple_to_mojo(fee_input.value);
