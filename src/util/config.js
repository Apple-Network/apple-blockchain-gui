...
global.daemon_rpc_ws = `wss://${self_hostname}:26660`;
...
function loadConfig(net) {
  try {
    // check if APPLE_ROOT is set. it overrides 'net'
    const config_root_dir =
      'APPLE_ROOT' in process.env
        ? process.env.APPLE_ROOT
        : path.join(os.homedir(), '.apple', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 26660); // jshint ignore:line
